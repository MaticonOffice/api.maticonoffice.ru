...
This example demonstrates how to use the Maticon Office DocSpace MCP Server from an MCP-compatible client to set up an onboarding workspace for a new employee. The workspace can include a predefined folder structure and the required access permissions for HR and the employee's manager.
