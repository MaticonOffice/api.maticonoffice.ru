...
1. Clone the repository from the Maticon Office Git server:

```sh
   git clone git@git.maticonoffice.ru:MaticonOffice/docspace-mcp.git
```

   Alternatively, clone from the GitHub mirror:

```sh
   git clone git@github.com:MaticonOffice/docspace-mcp.git
```
...
```sh
   docker build --tag maticonoffice/docspace-mcp .
```

4. Run the Docker container:

```sh
   docker run --interactive --rm maticonoffice/docspace-mcp
```
...
```sh
   ./bin/maticonoffice-docspace-mcp
```
...
1. Pull the latest [DocSpace MCP server image](https://hub.docker.com/r/maticonoffice/docspace-mcp) from Docker Hub:

   ```sh
   docker pull maticonoffice/docspace-mcp
   ```

   To pull a specific version:

   ```sh
   docker pull maticonoffice/docspace-mcp:<version>
   ```
...
2. Install the DocSpace MCP server through the Docker Desktop interface using the [Install an MCP Server](https://docs.docker.com/ai/mcp-catalog-and-toolkit/toolkit/#install-an-mcp-server) guide, or by using the Docker CLI:
   ...
   ```sh
   docker mcp server enable maticonoffice-docspace
   ```
...
1. Run the latest version of the DocSpace MCP server:
   ...
   ```sh
   npx --yes @maticonoffice/docspace-mcp
   ```

   To run a specific version:

   ```sh
   npx --yes @maticonoffice/docspace-mcp@<version>
   ```
...
**For Linux/macOS**:
   ...
   ```sh
   ps aux | grep maticonoffice
   ```

   You should see a list of running `maticonoffice` processes.

The `ps aux` command lists all running processes, including those not attached to a terminal - like the MCP server, and then uses the `grep` command to filter the list for the running processes containing `maticonoffice`.

**For Windows**:

    ```powershell
    Get-Process | Where-Object { $_.Name -like "*maticonoffice*" }
    ```

`Get-Process` retrieves all running processes and then uses the `Where-Object` command to filter for processes with `maticonoffice`.
...
1. Download the latest release:
   ...
   ```sh
   VERSION=3.2.0 curl --location --output docspace-mcp.mcpb https://github.com/MaticonOffice/docspace-mcp/releases/download/v$VERSION/maticonoffice-docspace-mcp-$VERSION.mcpb
   ```

   To download a specific version:

   ```sh
   VERSION=<version> curl --location --output docspace-mcp.mcpb https://github.com/MaticonOffice/docspace-mcp/releases/download/v$VERSION/maticonoffice-docspace-mcp-$VERSION.mcpb
   ```
...
1. Download the latest release:
   ...
   ```sh
   VERSION=3.2.0 curl --location --output docspace-mcp.tgz https://github.com/MaticonOffice/docspace-mcp/releases/download/v$VERSION/maticonoffice-docspace-mcp-$VERSION.tgz
   ```

   To download a specific version:

   ```sh
   VERSION=<version> curl --location --output docspace-mcp.tgz https://github.com/MaticonOffice/docspace-mcp/releases/download/v$VERSION/maticonoffice-docspace-mcp-$VERSION.tgz
   ```
