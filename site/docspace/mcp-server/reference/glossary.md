...
`https://your-instance.maticonoffice.ru`) and contains all your rooms, files, and
...
HTTP connection. Used as an MCP transport (`sse`) and is available at `https://mcp.maticonoffice.ru/sse`. It is one-directional (server → client), and currently superseded by the bi-directional Streamable HTTP for MCP use.
...
`https://mcp.maticonoffice.ru/mcp`.
