...
- **Missing or misplaced `mcpServers` key.** Make sure the `maticonoffice-docspace` record is nested directly inside the `mcpServers` object, not at the root level of the file.
