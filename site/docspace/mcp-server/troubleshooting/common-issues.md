...
- Ensure the URL is entered correctly: `https://mcp.maticonoffice.ru/mcp`.
- Check that your network or firewall does not block access to `mcp.maticonoffice.ru`.
...
- **Pop-ups are blocked.** If the authentication page does not appear, check that your browser is not blocking pop-ups from `mcp.maticonoffice.ru`.
- **Session expired or token revoked.** Delete the existing `maticonoffice-docspace` entry, re-add it, and repeat the authentication process.
- **Corporate firewall or proxy.** If your organization uses a proxy, make sure that `mcp.maticonoffice.ru` is reachable. Contact your network administrator if needed.
...
```bash
curl -I https://your-instance.maticonoffice.ru
```
...
- **The bundle file is corrupted or from an outdated release:** Re-download the latest MCP bundle from [GitHub Releases](https://github.com/MaticonOffice/docspace-mcp/releases).
...
After editing the client `config.json` file, the client displays an error for the `maticonoffice-docspace` server, or the server is not listed among the available connections.

- **Docker is not running:** Make sure Docker Desktop (or the Docker daemon on Linux) is started before launching the client.
- **The Docker image is not pulled:** Run `docker pull maticonoffice/docspace-mcp` manually in a terminal to verify the image can be downloaded.
...
- Check that the Docker image was pulled successfully and the container can start. Sometimes, your Docker image may be outdated - run `docker pull maticonoffice/docspace-mcp` to update to the latest version of the image.
