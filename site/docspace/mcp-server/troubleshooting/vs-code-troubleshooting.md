...
2. Validate the JSON structure. Make sure the `maticonoffice-docspace` entry is nested inside the `servers` object and all brackets are properly closed.
...
**Local:** Docker is not running, or the `maticonoffice/docspace-mcp` image has not been pulled yet.

To fix this:

- **Remote:** Check your internet connection. Verify that `https://mcp.maticonoffice.ru/mcp` is reachable from your browser. If the issue persists, try restarting VS Code.
...
1. Open Command Palette, select **MCP: List Servers**, then select **maticonoffice-docspace** and choose **Stop Server**.
2. Wait a few seconds, then select **Start Server** again and re-authenticate.
3. If the problem repeats, sign out of your DocSpace account in the browser, clear cookies for `mcp.maticonoffice.ru`, and start the authentication flow again.
...
- Ensure the URL does not have a trailing slash (e.g., use `https://portal.maticonoffice.ru`, not `https://portal.maticonoffice.ru/`).
...
```bash
docker run --rm \
  --env DOCSPACE_BASE_URL=https://your-instance.maticonoffice.ru \
