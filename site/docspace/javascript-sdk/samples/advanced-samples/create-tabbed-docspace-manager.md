...
This example demonstrates how to embed Maticon Office DocSpace in a tabbed interface, where each tab runs its own DocSpace instance in manager mode.

Complete source code on GitHub: [Source](https://github.com/MaticonOffice/docspace-samples/blob/master/js-sdk/advanced-samples/create-tabbed-docspace-manager.html)
...
``` bash
REACT_APP_DOCPSACE_URL=https://yourportal.maticonoffice.ru
...
``` bash
npm i @maticonoffice/docspace-sdk-js @maticonoffice/docspace-react
```
- `@maticonoffice/docspace-react` mounts the iframe and wires SDK events.
- `@maticonoffice/docspace-sdk-js` provides methods such as `login()` (see Login, Get hash settings, Create hash samples for context).
