...
# Maticon Office API credentials
MATICONOFFICE_API_HOST = 'yourportal.maticonoffice.ru'
...
def get_docspace_groups():
    url = f'https://{MATICONOFFICE_API_HOST}/api/2.0/group'
...
def get_group_members(group_id):
    url = f'https://{MATICONOFFICE_API_HOST}/api/2.0/group/{group_id}'
    response = requests.get(url, headers=MATICONOFFICE_HEADERS)
    if response.status_code == 200:
        group = response.json()
        try:
