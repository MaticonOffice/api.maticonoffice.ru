...
def check_docspace_user(email):
    url = f"https://{MATICONOFFICE_API_HOST}/api/2.0/people/email?email={email}"
...
def add_docspace_user(first_name, last_name, email):
    url = f"https://{MATICONOFFICE_API_HOST}/api/2.0/people"
    data = {
    ...
    response = requests.post(url, json=data, headers=MATICONOFFICE_HEADERS)
    if response.status_code == 200:
