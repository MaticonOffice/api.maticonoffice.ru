...
This integration allows you to automatically import all file attachments from a specific Trello card into an Maticon Office DocSpace room. It's useful for syncing project files, archiving task assets, or aggregating external documentation into a centralized workspace.
...
MATICONOFFICE_API_HOST = "your-docspace.maticonoffice.ru"
MATICONOFFICE_API_KEY = "your_maticonoffice_api_key"
MATICONOFFICE_ROOM_ID = "your_room_id"
...
# Headers for Maticon Office authentication
MATICONOFFICE_HEADERS = {
    "Accept": "application/json",
    "Authorization": f"Bearer {MATICONOFFICE_API_KEY}",
...
def upload_to_docspace(file_content, filename, room_id):
    url = f"https://{MATICONOFFICE_API_HOST}/api/2.0/files/{room_id}/upload"
    files = {"file": (filename, io.BytesIO(file_content))}
    response = requests.post(url, headers=MATICONOFFICE_HEADERS, files=files)
...
if __name__ == '__main__':
    transfer_trello_to_docspace(TRELLO_BOARD_ID, TRELLO_CARD_NAME, MATICONOFFICE_ROOM_ID)
```
...
1. Replace `your-docspace.maticonoffice.ru`, `your_maticonoffice_api_key`, and `your_room_id` with your actual DocSpace portal host, API key, and room ID. Ensure you have the necessary permissions to perform user operations.
