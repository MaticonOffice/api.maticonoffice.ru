...
This integration creates collaboration rooms in Maticon Office DocSpace based on Asana projects and generates folders for each task inside the corresponding room. This setup is ideal for aligning your document structure with your project management workflow.
...
# Maticon Office DocSpace configuration
...
def create_docspace_room(room_title):
    url = f"https://{MATICONOFFICE_API_HOST}/api/2.0/files/rooms"
    payload = {
        "title": room_title,
        "roomType": 2,  # Collaboration room
        "private": True
    }
    response = requests.post(url, headers=MATICONOFFICE_HEADERS, json=payload)
    if response.status_code == 200:
...
def create_docspace_folder(parent_id, folder_name):
    url = f"https://{MATICONOFFICE_API_HOST}/api/2.0/files/folder/{parent_id}"
...
1. Replace `your-docspace.maticonoffice.ru` and `your_maticonoffice_api_key` with your actual DocSpace portal host and API key. Ensure you have the necessary permissions to perform user operations.
