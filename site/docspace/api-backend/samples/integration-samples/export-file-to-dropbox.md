...
def get_file_info(file_id):
    url = f"https://{MATICONOFFICE_API_HOST}/api/2.0/files/file/{file_id}"
    response = requests.get(url, headers=MATICONOFFICE_HEADERS)
...
def get_presigned_url(file_id):
    url = f"https://{MATICONOFFICE_API_HOST}/api/2.0/files/file/{file_id}/presigneduri"
    r = requests.get(url, headers=MATICONOFFICE_HEADERS)
