...
This example demonstrates how to enable the Custom Filter mode for a specific file in Maticon Office DocSpace using the API. The Custom Filter feature allows you to restrict spreadsheet views so that users only see their own applied filters.

Complete source code on GitHub: [Node.js](https://github.com/MaticonOffice/docspace-samples/blob/master/api-backend/nodejs/samples/enable-custom-filter.js)/[Python](https://github.com/MaticonOffice/docspace-samples/blob/master/api-backend/python/samples/enable-custom-filter.py)
