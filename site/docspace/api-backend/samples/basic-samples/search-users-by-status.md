...
This example demonstrates how to search for users in Maticon Office DocSpace based on their status (e.g., active, pending, disabled) using the API. Additional filters such as query text, department, or custom fields can be applied.

Complete source code on GitHub: [Node.js](https://github.com/MaticonOffice/docspace-samples/blob/master/api-backend/nodejs/samples/search-users-by-status.js)/[Python](https://github.com/MaticonOffice/docspace-samples/blob/master/api-backend/python/samples/search-users-by-status.py)
