...
Interacting with the DocSpace API requires authentication. Maticon Office offers several methods to authenticate your requests. The chosen authentication method depends on your application's purpose, its users, and the features it requires.
...
```sh
# Step 1: Get token
curl -X POST https://yourportal.maticonoffice.ru/api/2.0/authentication \
...
curl -u yourusername:yourpassword \
  https://yourportal.maticonoffice.ru/api/2.0/people/@self
```
