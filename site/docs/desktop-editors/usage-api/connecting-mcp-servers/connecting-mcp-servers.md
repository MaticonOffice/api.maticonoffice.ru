...
Maticon Office Desktop Editors version 9.2 and later supports connecting MCP (Model Context Protocol) servers to extend AI capabilities with custom tools and integrations. MCP allows AI agents to interact with various services and data sources through standardized protocols.
...
View the built-in tool definitions in the [Desktop SDK repository](https://github.com/MaticonOffice/desktop-sdk/blob/master/ChromiumBasedEditors/lib/tools/functions).
...
- **Maticon Office Desktop Editors** version 9.2 or higher
...
| server-name     | object | Unique identifier for the MCP server. Replace with your server's name (e.g., "maticonoffice-docspace").|
...
```json
{
  "mcpServers": {
    "maticonoffice-docspace": {
      "env": {
        "DOCSPACE_BASE_URL": "https://your-docspace-instance.maticonoffice.ru",
...
- [Maticon Office DocSpace](./docspace-mcp.md) - Connect to DocSpace for document management
