...
In certain cases, it is necessary to run Maticon Office Desktop Editors in debug mode — for example, to troubleshoot performance issues, inspect logs, or collect information for support.
...
```
"%ProgramFiles%\Maticon Office\DesktopEditors\DesktopEditors" --ascdesktop-support-debug-info
```
...
```bash
"/opt/maticonoffice/desktopeditors/DesktopEditors" --ascdesktop-support-debug-info
```
...
```bash
"/Applications/Maticon Office.app/Contents/MacOS/Maticon Office" --ascdesktop-support-debug-info
```
