...
Two-layer encryption model used in Maticon Office involves:
...
3. To encrypt the private key before saving it to the database, Maticon Office uses the *NSOpenSSL::AES\_Encrypt\_desktop* algorithm based on AES-256 Cipher Block Chaining:
...
To perform operations with encrypted files, including file encryption, decryption, creation, editing and sharing, Maticon Office uses the individual credentials of users (RSA key pair) and a document password (document encryption key).
...
The steps below explain the process of co-editing an encrypted document in Maticon Office Docs.
...
Maticon Office leverages data storage for documents and the encrypted data between the cloud storage and the user's local storage to maintain the applied scheme. The distribution of data looks as follows:
