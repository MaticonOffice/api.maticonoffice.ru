...
2. Fork the plugin marketplace repository: [maticonoffice.github.io](https://github.com/MaticonOffice/maticonoffice.github.io).

   You get a repository with the following address - `https://github.com/YOUR-USERNAME/maticonoffice.github.io`.
...
5. Add the plugin folder to `maticonoffice.github.io/sdkjs-plugins/content/` and push it to the remote repository.

   ![New folder](/assets/images/plugins/new-folder.png)

6. Open the `maticonoffice.github.io/store/plugin-dev/extension/inject.js` file and assign the path to your plugin on GitHub Pages site to the `URL_TO_PLUGIN` variable:

   ``` ts
   const URL_TO_PLUGIN = "https://YOUR-USERNAME.github.io/maticonoffice.github.io/sdkjs-plugins/content/helloworld/"
   ```

The `maticonoffice.github.io/store/plugin-dev/extension` folder is a new extension.

:::note
Please note that when adding an extension to [Maticon Office App Directory](https://www.maticonoffice.ru/app-directory/en), the **Title** and **Summary** are filled in automatically with the **name** and **description** parameters from the `manifest.json` file. You cannot change them later in the store.
:::
...
5. Launch Maticon Office Cloud and open any document to check if the new plugin is available in the editors:
...
The plugin will be removed from Maticon Office Cloud after page reload.
...
6. Launch Maticon Office Cloud and open any document to check if the new plugin is available in the editors:

   ![Hello world icon](/assets/images/plugins/helloworld_icon.png)

#### Removing plugins from Opera

To remove the plugin from the **Plugins** tab in Maticon Office editors, click the **Disable** button on the extension tab or turn off the switcher. You can enable the extension again at any time.
...
To remove the plugin from the **Plugins** tab in Maticon Office editors, disable it by turning off the switcher. You can enable the extension again at any time.
