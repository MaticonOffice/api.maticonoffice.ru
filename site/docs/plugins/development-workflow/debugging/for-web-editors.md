...
To debug Maticon Office plugins in the web editors, follow the instructions below,
...
When developing Maticon Office plugins, debugging is essential for identifying and resolving issues. The plugin environment has several specific characteristics that make debugging unique:
...
- Ensure [document server](https://www.maticonoffice.ru/download-docs#docs-community) is installed and accessible.
- Serve plugin resources over HTTPS when required by the editor.

**JWT**

- Verify the secret key in Document Server config: `%ProgramFiles%\Maticon Office\DocumentServer\config\local.json` (`services.CoAuthoring.secret.browser.string`)
