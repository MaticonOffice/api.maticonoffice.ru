...
```ini
[Service]
Environment="OLLAMA_ORIGINS=http://*,https://*,maticonoffice://*"
```
...
```bash
export OLLAMA_ORIGINS=http://*,https://*,maticonoffice://*
ollama serve
```
...
```bash
launchctl setenv OLLAMA_ORIGINS "http://*,https://*,maticonoffice://*"
```
...
```powershell
$env:OLLAMA_ORIGINS = "http://*,https://*,maticonoffice://*"
ollama serve
```
...
```powershell
setx OLLAMA_ORIGINS "http://*,https://*,maticonoffice://*"
```
...
```bash
docker run -d \
  -e OLLAMA_ORIGINS="http://*,https://*,maticonoffice://*" \
  -p 11434:11434 \
...
- `maticonoffice://*` - allows requests from Maticon Office embedded WebViews (for AI plugin functionality)
...
- Maticon Office Desktop with AI plugin
...
- **CORS headers with `always` flag** are mandatory for proper cross-domain request handling from Maticon Office and other browser clients. The `always` flag ensures headers are added even for error responses (4xx, 5xx).
...
- Maticon Office UI connects to Nginx via HTTPS protocol
...
- The Maticon Office AI plugin uses a secure HTTPS endpoint to connect to language models