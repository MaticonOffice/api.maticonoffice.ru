...
Maticon Office macros are different from Microsoft macros because the latter use the Visual Basic for Applications (VBA) scripting language. JavaScript is more flexible and can be used with any platform (which is important as Maticon Office editors are supported on Windows, Linux, and macOS).

This means that macros created in Microsoft Office are not compatible with Maticon Office macros. You can convert them for use with Maticon Office in two ways:
...
Starting from version 9.0, you can use the AI plugin built into Maticon Office to automatically convert VBA code into JavaScript compatible with Maticon Office macros:
...
2. **Office API mapping:** VBA operations are mapped to equivalent Maticon Office Office API methods.
3. **JavaScript macro generation:** A JavaScript macro compatible with Maticon Office macros is generated.
...
The same result can be achieved using Maticon Office macros. The code is almost identical if you know both VBA and JavaScript:
