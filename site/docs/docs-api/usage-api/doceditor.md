...
`DocsAPI.DocEditor` is the main class of Maticon Office Docs API. It is the entry point for creating, configuring, and managing a document editor embedded in a web page.

## DocsAPI

`DocsAPI` is the global namespace provided by the Maticon Office Docs [API script](../get-started/installation/self-hosted.md):
...
Replace `example.com` with the host of your **document storage service**. The [`callbackUrl`](./config/editor/editor.md#callbackurl) is the endpoint on your server where Maticon Office Docs sends document status updates and saved files. See the [How it works](../get-started/how-it-works/how-it-works.md) section to find out more on Maticon Office Docs service client-server interactions.
...
Install [`@maticonoffice/doceditor-types`](https://www.npmjs.com/package/@maticonoffice/doceditor-types) for full IntelliSense and type checking of the config object, `DocEditor` methods, and events. The package version tracks the Maticon Office Docs version.
:::
