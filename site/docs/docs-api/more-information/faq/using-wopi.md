...
## How can I configure the Maticon Office Docs to accept WOPI requests only from the trusted integrator?
...
Follow the steps [in this section](../../using-wopi/overview.md#ip-filter) to configure the Maticon Office Docs IP filter.
...
The requests are sent to the `https://documentserver/hosting/discovery` address where the **documentserver** is the name of the server with the Maticon Office Docs installed. You can [register](https://www.maticonoffice.ru/docs-registration.aspx?from=api) a free Maticon Office Cloud and use its public IP address or public DNS that can be found in the **Instances** section of the cloud console.
...
## Which WOPI REST API operations are supported by Maticon Office Docs?

Maticon Office Docs uses the following WOPI operations: [CheckFileInfo](../../using-wopi/wopi-rest-api/checkfileinfo.md), [GetFile](../../using-wopi/wopi-rest-api/getfile.md), [Lock](../../using-wopi/wopi-rest-api/lock.md), [RefreshLock](../../using-wopi/wopi-rest-api/refreshlock.md), [Unlock](../../using-wopi/wopi-rest-api/unlock.md), [PutFile](../../using-wopi/wopi-rest-api/putfile.md), [RenameFile](../../using-wopi/wopi-rest-api/renamefile.md). For more information about these operations, see the corresponding pages.

## How to restrict access to the use of Maticon Office server via WOPI?

You cannot restrict access to the use of Maticon Office server via WOPI. Editing access cotrol is available only via Maticon Office Docs API and based on JWT signature. The secret key is required to generate a [file open request](../../additional-api/signature/browser.md).
...
## What are the difference between Maticon Office Docs API and WOPI?

You can integrate [Maticon Office Docs](https://www.maticonoffice.ru/office-suite.aspx) with any cloud service using either API or WOPI.
