...
The document history can be shown using the [onRequestHistory](../../usage-api/config/events.md#onrequesthistory) function (with the *events.onRequestHistory* event). Unless you use them, the **Version History** menu option (**Version History** button in the **Collaboration** tab) is not shown in the Maticon Office Docs interface.
