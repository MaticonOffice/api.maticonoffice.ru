...
The **document editor** can be embedded in an Android application using the [WebView](https://developer.android.com/reference/android/webkit/WebView) component. A complete demo project is available on [GitHub](https://github.com/MaticonOffice/editors-webview-android).
...
2. Download the mobile demo sample for Android from [GitHub](https://github.com/MaticonOffice/editors-webview-android).

3. Open the project with [Android Studio](https://developer.android.com/studio).

4. In the module-level `build.gradle` file, set the `DOCUMENT_SERVER_URL` property to the address of your Maticon Office Docs instance:
...
5. The `MainFragment.kt` controller handles navigation from the **document manager** to the **document editor**. When the user taps a document, the controller intercepts the URL, checks whether it contains the `"editor"` string, and — if so — navigates to the editor fragment:
   ...
   The full code for `MainFragment.kt` can be found [here](https://github.com/MaticonOffice/editors-webview-android/blob/main/app/src/main/java/ru/mike/florida/MainFragment.kt).
...
The full code for `EditorFragment.kt` can be found [here](https://github.com/MaticonOffice/editors-webview-android/blob/main/app/src/main/java/ru/mike/florida/EditorFragment.kt).
