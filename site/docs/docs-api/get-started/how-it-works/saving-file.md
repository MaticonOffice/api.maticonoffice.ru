...
The figure and steps below explain how a document is saved in Maticon Office Docs.
...
In most cases, the save completes about 10 seconds after editing ends. The default delay is defined by the [`savetimeoutdelay`](../configuration/server-config/server-config.mdx#services.CoAuthoring.server.savetimeoutdelay) parameter in the [Maticon Office Docs server configuration](../configuration/server-config/server-config.mdx).

## Force saving

The **document editing service** allows you to get the current document state before editing is finished. This process is called *forcesave* in Maticon Office Docs. When forcesave is initiated, the **document editing service** sends a request to the [callback handler](../../usage-api/callback-handler.md) with the link to the document as the `url` parameter and the `status` parameter set to `6`. The forcesave process can be initiated in the following ways:
...
:::note
Document versions created by forcesave do not appear in the document history. Maticon Office Docs [highlights changes](./document-history.md#how-this-can-be-done-in-practice) made from the beginning of the current editing session, not from the beginning of each document version. Even if several document versions are created during one session, all changes from that session will be highlighted.
:::
...
:::warning
This setting can break integrations that open documents without prior conversion (for example, the `.doc` format, which is not available for saving in Maticon Office Docs). Disable this setting if necessary.
:::
