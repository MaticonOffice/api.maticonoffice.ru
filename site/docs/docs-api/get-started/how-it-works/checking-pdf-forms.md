...
To distinguish standard PDF files from PDF forms, you can use the check functions. These functions take the first 300 bytes of the file and check if there is `"MATICONOFFICEFORM"` there. If `true`, then it is a PDF form. Otherwise, it is a standard PDF file.
...
<Tabs>
    ...
    function isExtendedPDFFile(text) {
      ...
      const signature = "MATICONOFFICEFORM";
    ...
    public function isMaticonofficeForm($file) {
      if ($file === null) return false;

      $limitDetect = 300;
      $maticonofficeFormMetaTag = "MATICONOFFICEFORM";
      ...
      $indexMeta = strpos($pFirst, $maticonofficeFormMetaTag);
      if ($indexStream === false || $indexMeta === false || $indexStream < $indexMeta) {
        return false;
      }
      $pMeta = substr($pFirst, $indexMeta);
      $pMeta = substr($pMeta, strlen($maticonofficeFormMetaTag) + 3);
