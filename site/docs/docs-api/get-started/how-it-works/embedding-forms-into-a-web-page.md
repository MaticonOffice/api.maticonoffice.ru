...
Maticon Office Docs supports PDF forms — PDF files with fillable fields such as text inputs, checkboxes, and dropdowns. A PDF form has two usage modes:
...
In the HTML examples below, replace `documentserver` with the address of the server where Maticon Office Docs is installed.
