---
description: Integrate Maticon Office Docs into ownCloud for document editing and collaboration.
...
This [app](https://github.com/MaticonOffice/maticonoffice-owncloud) enables users to edit office documents from [ownCloud](https://owncloud.com) using Maticon Office Docs.

The latest app signed versions are available in the official store for [ownCloud](https://marketplace.owncloud.com/apps/maticonoffice).
...
Maticon Office Docs and ownCloud can be installed either on different computers, or on the same machine. In case you select the latter variant, you will need to set up a custom port for Document Server as by default both Maticon Office Docs and ownCloud work on port 80. Or you can use Maticon Office Docs behind a proxy, please refer to [this article](https://help.maticonoffice.ru/server/document/document-server-proxy.aspx) to learn how you can configure it.

You can also use our [Docker installation](https://github.com/MaticonOffice/docker-maticonoffice-owncloud) to get installed and configured Maticon Office Docs and ownCloud installation with a couple of commands.

## Installing Maticon Office app for ownCloud

The ownCloud administrator can install the integration app from the in-built application market. To do this, click the upper-left hamburger menu and select **Market**. After that, find **Maticon Office** in the list of available applications and install it.

If the server with the ownCloud installed does not have the Internet access, or if you need it for some other reason, the administrator can install the application manually. To start using Maticon Office Docs with ownCloud, the following steps must be performed:
...
2. Get the Maticon Office app for ownCloud. There are several ways to do that:

   1. Download the latest signed version from the official store for [ownCloud](https://marketplace.owncloud.com/apps/maticonoffice).
   2. Download the latest signed version from the application [release page](https://github.com/MaticonOffice/maticonoffice-owncloud/releases) on GitHub.
   3. Clone the application source code and compile it yourself:
   
   ``` sh
   git clone https://github.com/MaticonOffice/maticonoffice-owncloud.git maticonoffice
   cd maticonoffice
...
4. In ownCloud, open the *\~/settings/admin?sectionid=apps\&category=disabled* page with **Not enabled** apps by administrator and click **Enable** for the **Maticon Office** application.

## Configuring Maticon Office app for ownCloud

In ownCloud, open the *\~/settings/admin?sectionid=additional#maticonoffice* page with administrative settings for **Maticon Office** section. Enter the following address to connect Maticon Office Docs:
...
- If you installed ownCloud Web from the [official marketplace](https://marketplace.owncloud.com/apps/maticonoffice), the path is *\<owncloud-root-catalog>/config/config.json*.
- If you compiled it from source code yourself using [this instruction](#installing-maticonoffice-app-for-owncloud), the path is *\<owncloud-web-root-catalog>/config/config.json*.
...
```json
{
  "external_apps": [
    {
      "id": "maticonoffice",
      "path": "http(s)://<owncloud-10-server-address>/custom/maticonoffice/js/web/maticonoffice.js"
    }
  ]
}
...
3. When opening an existing file, the user navigates to it within ownCloud and selects the **Open in Maticon Office** menu option.
...
8. Then Maticon Office Docs downloads the document from ownCloud and the user begins editing.

9. Maticon Office Docs sends a POST request to *callbackUrl* to inform ownCloud that a user is editing the document.

10. When all users and client browsers are done with editing, they close the editing window.

11. After [10 seconds](../how-it-works/saving-file.md#save-delay) of inactivity, Maticon Office Docs sends a POST to *callbackUrl* letting ownCloud know that the clients have finished editing the document and closed it.
...
- **Validating certificate**. If you are using a self-signed certificate for your Maticon Office Docs, ownCloud will not validate such a certificate and will not allow connection to/from Maticon Office Docs. This issue can be solved in two ways:

  1. Check the **Disable certificate verification (insecure)** box on the Maticon Office administration page, Server settings section, within your ownCloud.
  ...
  This will disable the certificate verification and allow ownCloud to establish connection with Maticon Office Docs.
...
- **Background task**. If the editors don't open or save documents after a period of proper functioning, the reason can be a problem in changing network settings or disabling any relevant services, or issues with the SSL certificate.
  ...
  To solve this, we added an asynchronous background task which runs on the server to check availability of the editors. It allows testing the connection between your **ownCloud instance** and **Maticon Office Docs**, namely availability of server addresses and the validity of the JWT secret are being checked.

  If any issue is detected, the Maticon Office app (consequently, the ability to create and open files) will be disabled. As an ownCloud admin, you will get the corresponding notification.
...
Download the Maticon Office app for ownCloud [here](https://github.com/MaticonOffice/maticonoffice-owncloud).
