...
When Maticon Office Docs runs as a multi-server cluster, collaborative editing requires that all requests for the same document reach the same server node. The `shardkey` query parameter enables this: your load balancer can read it from the URL and route accordingly.
