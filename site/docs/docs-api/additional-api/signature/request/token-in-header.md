...
When performing the HTTP requests from or to Maticon Office Docs an authorization header with the token is added to validate the request.
...
:::caution
It is not recommended to send a signature in the header when performing the HTTP requests to Maticon Office Docs. Send the signature [in the body](./token-in-body.md).
:::
