...
Maticon Office Docs [uses](../../get-started/how-it-works/security.md) tokens generated using the JSON Web Tokens standard. The tokens are sent when performing the client-side [browser requests](browser.md) to Maticon Office Docs or the [HTTP requests](request/request.md) to or from Maticon Office Docs.

:::info
This feature is used in Maticon Office Docs starting with version 4.2.
:::

For the validation setup, it is necessary to edit the [secret key](https://help.maticonoffice.ru/installation/docs-developer-configuring.aspx#SecretKey) and [token](https://help.maticonoffice.ru/installation/docs-developer-configuring.aspx#Token) parameters in the configuration file, which can be found (or created) at the following path:
...
| ------------------------------------------------ | ------- | ------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| services.CoAuthoring.secret.browser.string       | string  | secret  | The secret key to generate a token in the client-side [browser requests](browser.md) to Maticon Office Docs.                                                                                                                                                                         |
