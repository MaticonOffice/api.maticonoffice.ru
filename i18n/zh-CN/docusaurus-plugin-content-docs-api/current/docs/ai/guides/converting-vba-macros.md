...
Maticon Office 宏与 Microsoft 宏不同，因为后者使用 Visual Basic for Applications (VBA) 脚本语言。JavaScript 更加灵活，可在任何平台上使用（这一点很重要，因为 Maticon Office 编辑器支持 Windows、Linux 和 macOS）。

这意味着在 Microsoft Office 中创建的宏与 Maticon Office 宏不兼容。您可以通过以下两种方式将其转换以在 Maticon Office 中使用：

- [自动转换](#automatic-conversion-using-ai-plugin) - 使用适用于 Maticon Office 的 AI 插件（推荐用于快速转换）。

- [手动转换](#manual-conversion-of-vba-macros) - 使用 [Maticon Office Office API](../../office-api/get-started/overview.md) 以 JavaScript 重写逻辑。

## 使用 AI 插件自动转换 {#automatic-conversion-using-ai-plugin}

从 9.0 版本开始，您可以使用 Maticon Office 内置的 AI 插件，自动将 VBA 代码转换为与 Maticon Office 宏兼容的 JavaScript 代码：
...
2. **Office API 映射：** 将 VBA 操作映射到等效的 Maticon Office Office API 方法。
3. **JavaScript 宏生成：** 生成与 Maticon Office 宏兼容的 JavaScript 宏。
...
使用 Maticon Office 宏可以实现相同的结果。如果您同时了解 VBA 和 JavaScript，代码几乎完全相同：
