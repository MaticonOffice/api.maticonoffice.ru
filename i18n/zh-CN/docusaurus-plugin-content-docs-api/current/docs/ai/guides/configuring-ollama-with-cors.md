...
- `maticonoffice://*` - 允许来自 Maticon Office 嵌入式 WebView 的请求 (用于 AI 插件功能)
...
- **带有 `always` 标志的 CORS 头**对于正确处理来自 Maticon Office 和其他浏览器客户端的跨域请求是必需的。`always` 标志确保即使对于错误响应 (4xx、5xx) 也添加头。
