...
2. 分叉插件市场仓库：[maticonoffice.github.io](https://github.com/MaticonOffice/maticonoffice.github.io)。

   你将获得一个地址为 `https://github.com/YOUR-USERNAME/maticonoffice.github.io` 的仓库。

3. 从该仓库构建你自己的 GitHub 页面网站。请参考官方 [GitHub 文档](https://docs.github.com/en/pages/quickstart) 了解详细操作。

4. 将分叉仓库克隆到本地计算机。

5. 将插件文件夹添加到 `maticonoffice.github.io/sdkjs-plugins/content/` 并推送到远程仓库。

   ![新文件夹](/assets/images/plugins/new-folder.png)

6. 打开 `maticonoffice.github.io/store/plugin-dev/extension/inject.js` 文件，将 GitHub Pages 网站中插件的路径赋值给 `URL_TO_PLUGIN` 变量：

   ``` ts
   const URL_TO_PLUGIN = "https://YOUR-USERNAME.github.io/maticonoffice.github.io/sdkjs-plugins/content/helloworld/"
   ```

`maticonoffice.github.io/store/plugin-dev/extension` 文件夹即为新的扩展。

:::note
请注意，当您将扩展程序添加至 [Maticon Office 应用目录](https://www.maticonoffice.ru/app-directory/zh)时，**标题**和 **总结**会根据 `manifest.json` 文件中的**名字**和**介绍**参数自动填写。您之后将无法在商店中修改这些信息。
:::
...
5. 启动 Maticon Office 云服务并打开任意文档，确认插件是否出现在编辑器中：
...
页面刷新后，该插件将从 Maticon Office 云服务中移除。
...
6. 启动 Maticon Office 云服务并打开任意文档，确认插件是否出现在编辑器中：

   ![Hello world 图标](/assets/images/plugins/helloworld_icon.png)

#### 从 Opera 中移除插件

要从 Maticon Office 编辑器的**插件**标签中移除插件，请点击扩展卡片中的**禁用**按钮或关闭开关。你可以随时重新启用扩展程序。
...
要从 Maticon Office 编辑器的**插件**标签中移除插件，请关闭扩展开关。你可以随时重新启用扩展程序。
