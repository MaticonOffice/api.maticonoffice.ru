---
sidebar_position: 0
description: 了解如何使用提供的 CSS 类和组件来设计 Maticon Office 插件界面的样式。
---

# 样式

Maticon Office 提供了用于不同界面元素的样式表。若要使你的插件界面适配 Maticon Office 的样式，可以在 [index.html](entry-point.md) 文件中通过以下链接引入 [plugin.css](https://maticonoffice.github.io/sdkjs-plugins/v1/plugins.css) 文件：

```html
<link rel="stylesheet" href="https://maticonoffice.github.io/sdkjs-plugins/v1/plugins.css">
```

为了保持用户插件界面的一致性，您可以使用一组专为生成插件 UI 设计的预构建基础组件。您可以在我们的 [Storybook](https://maticonoffice.github.io/storybook/static/) 中探索组件功能，并进行交互。

所有可用的控件也都在[**控件示例**](https://github.com/MaticonOffice/sdkjs-plugins/tree/master/example_controls)插件中展示：
...
此示例演示如何在 YouTube 插件中添加符合 Maticon Office 风格的界面元素。
