...
3. 为了在将私钥保存到数据库之前对其进行加密，Maticon Office 使用基于 AES-256 密码块链（Cipher Block Chaining）的 *NSOpenSSL::AES_Encrypt_desktop* 算法：
...
为了执行加密文件的相关操作（包括文件加密、解密、创建、编辑和共享），Maticon Office 使用用户的个人凭据（RSA 密钥对）和文档密码（文档加密密钥）。
...
以下步骤解释了在 Maticon Office Docs 中协同编辑加密文档的过程。
...
Maticon Office 利用云存储和用户本地存储之间的文档和加密数据存储来维持所应用的方案。数据分布如下：
...
- 从 *sdkjs-plugins* 目录中删除 *cloud_crypto.xml* 文件。文件夹路径取决于所使用的操作系统：
  - Linux 系统 - *home/\<username>/.local/share/maticonoffice/desktopeditors/sdkjs-plugins/*
  - Windows 系统 - *C:\Users\\\<username>\AppData\Local\Maticon Office\DesktopEditors\data\sdkjs-plugins\\*
