...
Maticon Office DocSpace MCP 服务器允许桌面编辑器中的 AI 代理与您的 DocSpace 实例交互，通过自然语言命令实现文档管理操作。

## 先决条件

在连接 DocSpace MCP 服务器之前，请确保您具有：

- **Maticon Office 桌面编辑器** 9.2 或更高版本
- 访问您的 Maticon Office DocSpace 实例（SaaS 或本地部署）
- DocSpace 的有效 API 密钥
- 在您的计算机上安装并运行 Docker
...
- **DOCSPACE_BASE_URL**：DocSpace 实例的完整 URL（例如 `https://your-company.maticonoffice.ru`）
...
- 检查 Docker 守护进程状态
- 确保 Docker 镜像可用：`docker pull maticonoffice/docspace-mcp`
...
- 保存配置后重新启动 Maticon Office 桌面编辑器
...
- [Maticon Office DocSpace API 文档](/docspace)
- [模型上下文协议规范](https://modelcontextprotocol.io)
- [Maticon Office 博客：将 DocSpace MCP 服务器连接到桌面编辑器](https://www.maticonoffice.ru/blog/2025/12/connect-maticonoffice-docspace-mcp-server-to-desktop-editors)
