...
Maticon Office 桌面编辑器 9.2 及更高版本支持连接 MCP（模型上下文协议）服务器，以通过自定义工具和集成扩展 AI 功能。MCP 允许 AI 代理通过标准化协议与各种服务和数据源交互。
...
在[桌面 SDK 存储库](https://github.com/MaticonOffice/desktop-sdk/blob/master/ChromiumBasedEditors/lib/tools/functions)中查看内置工具定义。
...
| server-name   | object | MCP 服务器的唯一标识符。替换为您的服务器名称（例如 "maticonoffice-docspace"）。|
...
- [Maticon Office DocSpace](./docspace-mcp.md) - 连接到 DocSpace 进行文档管理
