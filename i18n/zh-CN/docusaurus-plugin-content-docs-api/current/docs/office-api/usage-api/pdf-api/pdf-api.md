...
要使用 **Maticon Office Document Builder** 创建 PDF 文档，您首先需要创建可编辑格式（DOCX）的文档，然后将其保存为 PDF。最简单的示例创建一个包含居中的 "Center" 文本的单页文档：

```ts
// 使用 Maticon Office Document Builder 创建文档文件
...
```ts
// 使用现有 'mydocument.pdf' 文件的路径或 URL 通过 Maticon Office Document Builder 打开它
