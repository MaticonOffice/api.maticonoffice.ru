...
Maticon Office Document Builder 允许将文档文件保存为图像文件，创建文档第一页或所有页面的缩略图。这是通过 `SaveFile` 方法的参数完成的。参数以 XML 标签的形式添加，可以使用以下标签：
