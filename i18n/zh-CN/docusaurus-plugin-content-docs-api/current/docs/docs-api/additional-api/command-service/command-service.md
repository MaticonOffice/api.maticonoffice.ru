...
与**文档命令服务**的交互使用 POST 请求。请求参数以 JSON 格式填写在请求正文中。请求被发送至`https://documentserver/command`地址，其中**文档服务器**指安装了 Maticon Office 文档的服务器名称。
...
:::note
在版本4.2之前的 **Maticon Office 文档服务器**中，使用了带有查询字符串中参数的GET请求。
:::
...
| [license](license.md)                   | 此命令可用于向 Maticon Office 文档请求许可证，并获取有关服务器和用户配额的信息。                  |
| [meta](meta.md)                         | 此命令允许为所有协作编辑器更新文档的元信息。                                  |
| [version](version.md)                   | 此命令可用于向 Maticon Office 文档请求当前版本号。                                                      |
...
| 名称  | 类型   | 是否必填                  | 描述                                                                                                                                                                    |
| ----- | ------ | ------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| token | string | 配置所需 | 以[令牌](../../additional-api/signature/request/token-in-body.md#get-document-status)形式添加到 **Maticon Office 文档**配置的加密签名。 |
