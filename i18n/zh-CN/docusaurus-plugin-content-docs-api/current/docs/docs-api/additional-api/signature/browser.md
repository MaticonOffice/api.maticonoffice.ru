...
在向 Maticon Office 文档服务器执行客户端浏览器请求时，必须将令牌添加到参数中以验证数据。
...
在 Maticon Office 文档中[打开](../../get-started/how-it-works/opening-file.md)文件进行编辑时，必须将[令牌](../../usage-api/config/config.md#token)添加到配置中以验证参数。
...
在 Maticon Office 文档中调用 [setHistoryData](../../usage-api/methods.md#sethistorydata) 方法查看文档[历史](../../get-started/how-it-works/document-history.md)版本时，必须添加令牌以验证参数。
