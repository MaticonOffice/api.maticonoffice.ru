...
Maticon Office 文档[使用](../../get-started/how-it-works/security.md)令牌，这些令牌是使用 JSON Web Tokens 标准生成的。当执行客户端[浏览器请求](browser.md)到 Maticon Office 文档，或与 Maticon Office 文档 之间进行 [HTTP 请求](request/request.md)时，会发送这些令牌。

:::info
此功能从 Maticon Office 文档 4.2 版本开始使用。
:::

对于验证设置，需要编辑位于配置文件中的 [secret key](https://help.maticonoffice.ru/installation/docs-developer-configuring.aspx#SecretKey) 和 [token](https://help.maticonoffice.ru/installation/docs-developer-configuring.aspx#Token)参数，配置文件可在以下路径找到(或创建)：
...
:::caution
默认值存储在 `default.json` 配置文件中，该文件位于上述文件夹中（适用于 Linux 和 Windows）。请不要直接编辑 `default.json` 文件的内容。每次重启 Docker 容器或将 Maticon Office 文档升级到新版本时，默认值将被恢复，您所做的所有更改将会丢失。
:::
...
| 参数                                        | 类型    | 示例 | 描述                                                                                                                                                                                                                                                                                                                                                                             |
| ------------------------------------------------ | ------- | ------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| services.CoAuthoring.secret.browser.string       | string  | secret  | 用于在客户端[浏览器请求](browser.md)中生成发送到 Maticon Office 文档的令牌的密钥。                                                                                                                        |
