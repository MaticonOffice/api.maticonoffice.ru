...
以下步骤解释了如何在同一个 `.html` 页面上嵌入多个Maticon Office 文档编辑器。
...
3. 在页面中引入Maticon Office 文档的 JavaScript API 脚本：

   ``` html
   <script type="text/javascript" src="https://documentserver/web-apps/apps/api/documents/api.js"></script>
   ```

   其中 `documentserver` 是安装了Maticon Office 文档的服务器名称。`api.js` 脚本由**文档编辑服务**提供，它会加载**文档编辑器**并将其连接到该服务。

   :::tip
   还没有文档服务器？[注册](https://www.maticonoffice.ru/zh/docs-registration.aspx?from=api)一个免费的Maticon Office 文档云，并使用实例的公共 IP 地址或公共 DNS 名称作为 `documentserver`。您可以在云控制台的**实例**部分找到它们。
   :::
