...
**文档编辑器**可以通过 [WebView](https://developer.android.com/reference/android/webkit/WebView) 组件嵌入到 Android 应用中。完整的演示项目可在 [GitHub](https://github.com/MaticonOffice/editors-webview-android) 上获取。

## 基于 Maticon Office 测试示例的集成

本示例演示如何将 Maticon Office 移动端网页编辑器与 Maticon Office [测试或 DMS 示例](../../../samples/language-specific-examples/language-specific-examples.md)集成。
...
2. 从 [GitHub](https://github.com/MaticonOffice/editors-webview-android) 下载 Android 移动演示示例。

3. 使用 [Android Studio](https://developer.android.com/studio) 打开项目。

4. 在模块级 `build.gradle` 文件中，将 `DOCUMENT_SERVER_URL` 属性设置为您的 Maticon Office 文档实例地址：

   ``` groovy
   buildConfigField("String", "DOCUMENT_SERVER_URL", "https://documentserver/")
   ```

   其中 `documentserver` 是已安装 Maticon Office 文档的服务器名称。

   :::tip
   还没有文档服务器？[注册](https://www.maticonoffice.ru/zh/docs-registration.aspx?from=api)免费的 Maticon Office 文档云，并使用实例的公共 IP 地址或公共 DNS 名称作为 `documentserver`。您可以在云控制台的**实例**部分找到它们。
   :::
...
5. `MainFragment.kt` 控制器负责从**文档管理器**到**文档编辑器**的导航。当用户点击文档时，控制器拦截 URL，检查是否包含 `"editor"` 字符串，若包含则导航到编辑器 fragment：
   ...
   `MainFragment.kt` 完整代码请参见[此处](https://github.com/MaticonOffice/editors-webview-android/blob/main/app/src/main/java/ru/mike/florida/MainFragment.kt)。
...
`EditorFragment.kt` 完整代码请参见[此处](https://github.com/MaticonOffice/editors-webview-android/blob/main/app/src/main/java/ru/mike/florida/EditorFragment.kt)。
