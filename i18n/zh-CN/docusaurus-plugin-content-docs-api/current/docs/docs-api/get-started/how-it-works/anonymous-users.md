...
以下参考图和步骤解释了在Maticon Office 文档中为匿名用户设置名称的过程。
...
要限制匿名用户对编辑器或实时查看器的访问，请将服务器配置文件中的[`services.CoAuthoring.server.isAnonymousSupport`](https://help.maticonoffice.ru/installation/docs-developer-configuring.aspx#services-CoAuthoring-server-isAnonymousSupport)参数设置为`false`。当匿名用户尝试打开编辑器时，将显示警告：
