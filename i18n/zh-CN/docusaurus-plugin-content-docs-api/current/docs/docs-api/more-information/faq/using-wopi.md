...
要启用 WOPI，请将 Maticon Office 文档配置中的 [wopi.enable](https://help.maticonoffice.ru/installation/docs-developer-configuring.aspx#wopi-enable) 参数设置为 **true**。
...
## 如何将 Maticon Office 文档配置为仅接受来自受信任的集成商的 WOPI 请求？ {#how-can-i-configure-the-maticonoffice-docs-to-accept-wopi-requests-only-from-the-trusted-integrator}
...
按照[此章节中](../../using-wopi/overview.md#ip-filter)的步骤配置 Maticon Office 文档 IP 过滤器。
...
请求被发送到`https://documentserver/hosting/discovery` 地址，其中 **documentserver** 是安装了 Maticon Office 文档的服务器的名称。您可以[注册](https://www.maticonoffice.ru/zh/docs-registration.aspx?from=api)一个免费的 Maticon Office 云，并使用其公共 IP 地址或公共 DNS，这些地址或 DNS 可以在云控制台的**实例**部分找到。
...
## Maticon Office 文档支持哪些 WOPI REST API 操作？ {#which-wopi-rest-api-operations-are-supported-by-maticonoffice-docs}

Maticon Office 文档使用以下 WOPI 操作：[CheckFileInfo](../../using-wopi/wopi-rest-api/checkfileinfo.md), [GetFile](../../using-wopi/wopi-rest-api/getfile.md), [Lock](../../using-wopi/wopi-rest-api/lock.md), [RefreshLock](../../using-wopi/wopi-rest-api/refreshlock.md), [Unlock](../../using-wopi/wopi-rest-api/unlock.md), [PutFile](../../using-wopi/wopi-rest-api/putfile.md), [RenameFile](../../using-wopi/wopi-rest-api/renamefile.md)。有关这些操作的更多信息，请参见相应页面。

## 如何通过WOPI限制访问Maticon Office服务器的使用？ {#how-to-restrict-access-to-the-use-of-maticonoffice-server-via-wopi}

您不能通过 WOPI 限制对 Maticon Office 服务器的使用。编辑访问控制只能通过 Maticon Office 文档 API 并基于 JWT 签名。生成[文件打开请求](../../additional-api/signature/browser.md)需要密钥。
...
## Maticon Office 文档 API 和 WOPI 有什么区别？ {#what-are-the-difference-between-maticonoffice-docs-api-and-wopi}

Y您可以使用 API 或 WOPI 将 [Maticon Office 文档](https://www.maticonoffice.ru/office-suite.aspx) 与任何云服务集成。
