...
- 在 **default.json** Maticon Office 文档配置文件中启用重复强制保存启动：
...
请参阅[保存文件](../../get-started/how-it-works/saving-file.md)部分以了解文件保存在Maticon Office 文档中是如何工作的，以及保存创建的文档需要什么。
...
编辑结束和转换开始之间的时间在default.json配置文件中的**用 [services.CoAuthoring.server.savetimeoutdelay](https://help.maticonoffice.ru/installation/docs-developer-configuring.aspx#services-CoAuthoring-server-savetimeoutdelay) 参数（默认为5000毫秒或5秒）进行设置，并可以更改：
