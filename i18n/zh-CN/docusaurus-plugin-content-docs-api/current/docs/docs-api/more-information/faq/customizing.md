...
## 如何对 Maticon Office 文档进行白标？ {#how-to-whitelabel-maticonoffice-docs}

以下选项可用于定制 Maticon Office 文档版本（白标化），修改**关于**页面显示信息（所有编辑器用户可见）：

:::note
请注意，更改这些选项仅适用于 [Maticon Office 文档开发者版](https://www.maticonoffice.ru/developer-edition-prices.aspx)用户。
:::
...
## 如何更改 Maticon Office 文档界面的默认语言？ {#how-to-change-the-default-language-of-the-maticonoffice-docs-interface}

如果界面语言出现在[可用的 Maticon Office 文档语言列表](https://help.maticonoffice.ru/docs/docs-available-languages.aspx)中，您可以简单地使用配置文件中的 *editorConfig.lang* 属性将其设置为必要的语言环境，如下所示：
...
若当前语言未包含在可用选项中，且您需要自行翻译或已完成自定义语言翻译，请参阅[此文档[此文](https://help.maticonoffice.ru/docs/installation/docs-community-add-language-docker.aspx)了解如何将其安装到 Maticon Office 文档中。
...
## 如何将插件连接到 Maticon Office 文档以便所有编辑器用户都可以看到它们？ {#how-to-connect-plugins-to-maticonoffice-docs-so-that-they-become-visible-for-all-editor-users}

要为所有 Maticon Office 文档用户启用插件，您需要参考以下内容：

- 获取插件（这可以使用我们在 [GitHub](https://github.com/MaticonOffice/sdkjs-plugins) 上的插件存储库来完成，或者您可以编写自己的插件）。
- 将插件放到文档编辑器可访问的文件夹中。
- 通过以下方式更改配置文件：
...
其中 *editorConfig.plugins.autostart* 是您希望在 Maticon Office 文档启动时自动启动的插件的 GUID 数组（在插件 [config.json](../../../plugins/configuration/configuration.md#guid) 文件中定义），*editorConfig.plugins.pluginsData* 是您希望 Maticon Office 文档用户可以使用的所有插件的 **config.json** 配置文件数组。

有关启用 Maticon Office 文档插件的更多信息，请参见[此页面](../../usage-api/config/editor/plugins.md)。
