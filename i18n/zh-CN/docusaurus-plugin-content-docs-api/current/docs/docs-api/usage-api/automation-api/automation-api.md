...
自动化 API 允许通过自定义界面元素从外部源与办公文档进行交互。利用 Maticon Office 文档处理功能构建您自己的 UI 组件 — 管理评论、控制审阅工作流程、自动填写表单等，所有操作都在编辑器外部完成。

:::info
自动化 API 仅适用于 **Maticon Office 文档开发者版**。

这是一项高级功能，默认不包含，需额外付费。请联系我们的销售团队 [sales@maticonoffice.ru](mailto:sales@maticonoffice.ru) 获取报价，或访问 [maticonoffice.ru/automation-api](https://www.maticonoffice.ru/automation-api) 了解更多信息。
:::
