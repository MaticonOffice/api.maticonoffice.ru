...
```ts
function onAppReady() {
  console.log("Maticon Office Document Editor is ready");
}
...
function onDownloadAs(event) {
  ...
  console.log(`Maticon Office Document Editor create file: ${url}`);
}
...
| 参数                         | 类型   | 描述         |
| ---------------------------- | ------ | ------------ |
| event.data.errorCode         | `number` | [错误代码](https://github.com/MaticonOffice/sdkjs/blob/master/common/errorCodes.js)。   |
...
function onError(event) {
  console.log(`Maticon Office Document Editor reports an error: code ${event.data.errorCode}, description ${event.data.errorDescription}`);
}
...
function onInfo(event) {
  console.log(`Maticon Office Document Editor is opened in mode ${event.data.mode}`);
}
...
:::note
此事件仅适用于 [Maticon Office 文档企业版](https://www.maticonoffice.ru/docs-enterprise-prices.aspx?from=api)和 [Maticon Office 文档开发者版](https://www.maticonoffice.ru/developer-edition-prices.aspx?from=api)。
:::
:::danger[Deprecated]
...
function onRequestEditRights() {
  console.log("Maticon Office Document Editor requests editing rights");
...
当用户尝试通过单击*数据*选项卡的*外部链接*对话框中的*更新值*按钮来刷新从外部文件插入的数据时调用的函数。要刷新数据，请调用 [setReferenceData](../methods.md#setreferencedata) 方法。调用该方法时，必须添加 `token` 来验证参数。当用户运行 [IMPORTRANGE](https://help.maticonoffice.ru/maticonoffice-editors/maticonoffice-spreadsheet-editor/Functions/importrange.aspx?from=api) 函数时，也会触发此事件。
...
| 参数                           | 类型   | 描述         |
| ------------------------------ | ------ | ------------ |
| event.data.warningCode         | `number` | [警告代码](https://github.com/MaticonOffice/sdkjs/blob/master/common/errorCodes.js)。   |
...
function onWarning(event) {
  console.log(`Maticon Office Document Editor reports a warning: code ${event.data.warningCode}, description ${event.data.warningDescription}`);
}
