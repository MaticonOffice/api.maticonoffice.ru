...
interface MaticonOfficeEditorProps {
...
const createDocumentConfig = (fileType: string, templateUrl: string, title?: string, isDemo: boolean = false, isForm: boolean = false): object => {
  return {
    ...
    url: templateUrl ? templateUrl : `https://static.maticonoffice.ru/assets/docs/samples/${getDocumentName(fileType, isDemo, isForm)}.${fileType}`
  };
};
...
const addScript = async (secret: string, fileType: string, code: string, theme: string, templateUrl: string, zoom: number,
                         externalConfig?: code, externalScript?: MaticonOfficeEditorProps["externalScript"], isDemo: boolean = false, isForm: boolean = false): Promise<void> => {
...
const MaticonOfficeEditor: React.FC<MaticonOfficeEditorProps> = ({
  fileType,
  code,
  height = "700px",
  templateUrl,
  ...
  useEffect(() => {
    if ("1" !== document.documentElement.getAttribute("data-script-api-state")) {
      if ("2" !== document.documentElement.getAttribute("data-script-api-state")) {
        ...
        scriptApi.onerror = () => {
          console.error("Failed to load MaticonOffice API script.");
        };
        scriptApi.onload = () => {
...
export default MaticonOfficeEditor;